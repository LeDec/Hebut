    ...
    >
      ...
      <el-form ref="form"  :inline="true" :model="form" label-width="90px">
        <el-form-item label="题名" prop="name">
          <el-input placeholder="请输入题名" v-model="form.name"></el-input>
        </el-form-item>
        <el-form-item label="出版社" prop="publicsh">
          <el-input placeholder="请输入出版社" v-model="form.publicsh"></el-input>
        </el-form-item>
        <el-form-item label="出版时间" prop="publishDate">
          <el-input placeholder="请输入出版时间" v-model="form.publishDate"></el-input>
        </el-form-item>
        <el-form-item label="专著类型" prop="type">
          <el-input placeholder="请输入专著类型" v-model="form.type"></el-input>
        </el-form-item>
...
<script>
import {deletemo, updatemo, insertmo, Selectmo} from '../../api/index'
...
export default {
  ...
  methods: {
    submit() {
      this.$refs.form.validate((valid) => {
        ...
        if (valid) {
          // 处理
          if (this.modelType === 0) {
            insertmo(this.form).then(() => {
              console.log(this.form)
              //更新完数据重新获取列表接口
              this.getList()
            })
          } else {
            updatemo(this.form).then(() => {
              console.log(this.form)
              //更新完数据重新获取列表接口
              this.getList()
            })
    ...
    handleDelete(row) {
      this.$confirm('此操作将永久删除该专著, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deletemo({type:"monograph",id:row.id}).then(()=>{
          this.$message({
            // type: 'success',
            // message: '删除成功!'
            type:'info',
    ...
    getList() {
      ...
      Selectmo({userid:global.UserId}).then(({data}) => {
        console.log(data)
