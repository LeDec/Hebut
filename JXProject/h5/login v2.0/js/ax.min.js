                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.code == 0) {
                            ...
                            if(data.data.type==5)
                                window.location.href = "../Student_New/home.html"
                            else if(data.data.type==1)
                                window.location.href = "../Administrator_New/home.html"
                            else if(data.data.type==2)
                                window.location.href = "../Headmaster_New/home.html"
                                 else if(data.data.type==4)
                                window.location.href = "../Expert_New/home.html"
                            else if(data.data.type==3)
                                window.location.href = "../teacher_New/home.html"
                        }
                        else {
                            alert("用户名或密码错误");
...
//注册
        ...
        //学生注册
        var btn = document.getElementById("s_enroll");
        btn.onclick = function () {
        var pwdRegex = new RegExp('(?=.*[0-9])(?=.*[a-zA-Z]).{8,30}');
        if (!pwdRegex.test($("#s_password").val())) {
        alert("您的密码复杂度太低（密码中必须包含字母、数字,且长度必须为8到30字符）！");
                ...
                data: JSON.stringify({
                 "email": $("#s_email").val(),
                 "instituteId":$("#s_instituteId").find("option:selected").val(),
                 "majorId":$("#s_majorId").find("option:selected").val(),
                 "name": $("#s_name").val(),
                 "password":$("#s_password").val(),
                 "passwordRepeat":$("#s_passwordRepeat").val(),
                 "userId": $("#s_userId").val()
                }),
  ...
  contentType: 'application/json',
        ...
        success:function(data){
             for (i in data.data){
                ...
                $("#s_majorId").append(('<option value='+data.data[i].majorId+'>'+'&nbsp&nbsp&nbsp&nbsp&nbsp'+data.data[i].majorName+'</option>'));
            }
        },
        error:function(data){
            console.log(data);
...
//教师注册
//点击注册验证
        var btn = document.getElementById("t_enroll");
        btn.onclick = function () {
        var pwdRegex = new RegExp('(?=.*[0-9])(?=.*[a-zA-Z]).{8,30}');
        if (!pwdRegex.test($("#t_password").val())) {
        alert("您的密码复杂度太低（密码中必须包含字母、数字,且长度必须为8到30字符）！");
        }
        else{
                $.ajax({
                url: 'http://'+localStorage.getItem('IP')+':'+localStorage.getItem('port')+'/project/user/registerTeacher',
                data: JSON.stringify({
                 "email": $("#t_email").val(),
                 "instituteId":$("#t_instituteId").find("option:selected").val(),
                  "userType":$("#t_userType").find("option:selected").val(),
                 "name": $("#t_name").val(),
                 "password":$("#t_password").val(),
                 "passwordRepeat":$("#t_passwordRepeat").val(),
                 "userId": $("#t_userId").val()
                }),
                contentType: 'application/json',
                type: 'post',
 ...
 document.getElementById('getCode').addEventListener("click",function(){
  ...
  var btn = document.getElementById("find_password");
         btn.onclick = function () {
              $.ajax({
