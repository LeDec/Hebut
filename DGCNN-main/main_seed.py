...
def main_LOCV():
    ...
    if os.path.exists(model_path):
        ...
        model, acc_all, pred_best_all, label_best_all = torch.load(model_path)
        print('model loaded.')
    else:
        for sub_i in range(sub_num):
            ...
            acc_test_best, m_pred, m_label = train(train_iter, test_iter, model, criterion, optimizer, num_epochs, file_list[sub_i])
        ...
        torch.save([model, acc_all, pred_best_all, label_best_all], model_path)
        print('model saved.')
    ...
    draw_confusion_matrix(label_true=label_best_all,
                          ...
                          pdf_save_path='./result/confusion_matrix/seed.svg',
                          dpi=300)
